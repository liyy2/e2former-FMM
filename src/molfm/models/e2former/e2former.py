...
class E2AttentionArbOrder_sparse(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_smooth_softplus = "use_smooth_softplus" in tp_type 
    ...
    def forward(
        ...
        **kwargs,
        ...
        elif self.use_smooth_softplus:
            # softplus version         
            alpha = torch.nn.functional.softplus(alpha)          # >= 0
            alpha = alpha * poly_dist.unsqueeze(-1)
            # mask invalid
            alpha = alpha.masked_fill(attn_mask, 0)
...
def construct_radius_neighbor(node_pos, node_mask,
                              ...
                              remove_self_loop_cluster=False,
                              error_check=False):
    '''
    node_pos: [B, L1, 3]
    node_mask: [B, L1]  1 means nodes, 0 means padding
    expand_node_pos: [B, L2, 3]
    expand_node_mask: [B, L2]  1 means clusters, 0 means padding
    radius: float
    cluster_ids: [B, L1]
    outcell_index: B*L2
    max_neighbors: int
    ...
    if remove_self_loop:
        dist = torch.where(dist < 1e-4, 1e4, dist)
    ...
    if cluster_ids is not None and remove_self_loop_cluster:
        cluster_ids_expand = cluster_ids.unsqueeze(-1)
        self_mask = (neighbor_indices == cluster_ids_expand)
        dist = dist.masked_fill(self_mask, 1e4)
 

    attn_mask = ~(dist <= radius) #| (dist < 1e-4)
    f_attn_mask = attn_mask[node_mask].unsqueeze(dim=-1)
    f_dist = dist[node_mask]  # flattn_N* topK*
    f_poly_dist = mace_polynomial(
        f_dist, radius
    )
    ...
    if error_check:
        ...
        if torch.max(a1) != 0:
            print("error: neighbor selection mismatch")
        if torch.sum(node_pos[~node_mask] == 0):
            print("Warning: node padding part is zero, may cause sort error")
        if torch.sum(expand_node_pos[~expand_node_mask] == 0):
            print("Warning: expand_node padding part is zero, may cause sort error")
...
def weighted_cluster_scatter(f_node_irreps, f_node_pos, f_cluster_pos, flat_cluster_ids, alpha=1.0):
    """
    f_node_irreps: [B*N, L, D]
    f_node_pos:    [B*N, 3]
    f_cluster_pos: [B*max_clusters, 3]
    flat_cluster_ids: [B*N] cluster id
    alpha: Gaussian decay weight factor

    return:
        f_cluster_irreps: [B*max_clusters, L, D]
    """
    B_N, L, D = f_node_irreps.shape
    # calculate dist
    node_cluster_pos = f_cluster_pos[flat_cluster_ids]  # [B*N, 3]
    dist = torch.norm(f_node_pos - node_cluster_pos, dim=-1, keepdim=True)  # [B*N, 1]

    # gaussian weight decay
    weight = torch.exp(-alpha * dist ** 2)  # [B*N, 1]

    # weight ongly on the scalar channel
    weighted_features = f_node_irreps.clone()
    weighted_features[:, 0:1, :] *= weight.unsqueeze(-1)
    # weighted_features *= weight.unsqueeze(-1)

    # aggregate on cluster
    weighted_features_flat = weighted_features.reshape(B_N, L*D)  # [B*N, L*D]
    sum_weighted = scatter_sum(weighted_features_flat, flat_cluster_ids, dim=0)  # [B*max_clusters, L*D]

    # normalization on the scalar dimension
    sum_weights = scatter_sum(weight, flat_cluster_ids, dim=0)  # [B*max_clusters, 1]

    sum_weights_expand = torch.cat([sum_weights] + [torch.ones_like(sum_weights)]*(L-1), dim=1)  # [B*max_clusters, L]
    sum_weights_expand = sum_weights_expand.repeat_interleave(D, dim=1)  # [B*max_clusters, L*D]

    f_cluster_irreps = (sum_weighted / (sum_weights_expand + 1e-6)).reshape(-1, L, D)

    return f_cluster_irreps
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pbc_max_radius!=max_radius:
            raise ValueError(" please ensue these two radius equal for pbc and non-pbc genelize")
        ...
        self.fmm_value_head_dim = int(kwargs.pop("fmm_value_head_dim", 8))
...
class E2formerCluster(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # )
        self.pre_norm_concat = True

        self.long_cutoff_upper = 15
        ...
        self.norm_cluster_short = get_normalization_layer(
            norm_layer, lmax=self.lmax, num_channels=self.scalar_dim
        )

        self.norm_cluster_long = get_normalization_layer(
            norm_layer, lmax=self.lmax, num_channels=self.scalar_dim
        )
        ...
        self.norm_ffn_cluster = torch.nn.ModuleList()
        for i in range(self.long_short_layers):
            ...
            self.norm_ffn_cluster.append(nn.Sequential(
                                    get_normalization_layer(
                                                norm_layer, lmax=self.lmax, num_channels=self.irreps_node_input[0][0]
                                                ),
                                    FeedForwardNetwork_s3(self.irreps_node_input[0][0],
    ...
    def forward(
        ...
        **kwargs,
        ...
        cluster_ids = torch.zeros((B, N), dtype=torch.int, device=device)
        # num_clusters_per_mol = torch.zeros((B), dtype=torch.int, device=device)
        min_nodes_foreachGroup = 24
        max_clusters = int(N/min(min_nodes_foreachGroup, N // 2 + 1)) + 1
        ...
        for i in range(B):
            ...
            num_clusters = int(num_atoms / min(min_nodes_foreachGroup, num_atoms // 2 + 1))+1
            ...
            cluster_ids[i][node_mask[i]] = torch.tensor(cluster_ids_np, device=device)
            cluster_pos[i,:num_clusters] = torch.from_numpy(kmeans.cluster_centers_).to(device)
        ...
        flat_cluster_ids = (cluster_ids+cluster_ptr[:B].unsqueeze(dim = 1))[node_mask]
        f_cluster_irreps = scatter_mean(f_node_irreps.reshape(-1, L*D), flat_cluster_ids, dim=0).reshape(-1, L, D)  # [B*max_clusters, D]       

        node_cluster_neighbor = construct_radius_neighbor(node_pos,node_mask,
                              cluster_pos,cluster_mask,
                              radius = self.long_cutoff_upper,
                              cluster_ids=cluster_ids,
                              outcell_index = None,
                              max_neighbors = self.max_neighbors,
                              remove_self_loop=False,
                              remove_self_loop_cluster=True,
                              error_check=False
                              )
        ...
        batched_data.update(
            {
                ...
                "exp_Y_powers": get_powers(f_cluster_pos,coeffs,self.lmax),
                "edge_vec_powers": torch.cat(get_powers(f_edge_vec,coeffs,self.lmax),dim = -2),
            }
        )
        f_node_irreps_short = f_node_irreps

        f_atomic_numbers = batched_data["masked_token_type"].reshape(B, N)[node_mask] 
        if self.lsrm_demo is not None:
            f_node_irreps,_=self.lsrm_demo(
                    node_pos=f_node_pos,
            ...
            for i, blk in enumerate(self.cluster_blocks):
                if self.use_smooth_fusion:
                    f_cluster_irreps = weighted_cluster_scatter(
                        f_node_irreps=f_node_irreps.reshape(-1, L, D),
                        f_node_pos=f_node_pos,
                        f_cluster_pos=f_cluster_pos,
                        flat_cluster_ids=flat_cluster_ids,
                        alpha=1.0
                    )
                elif self.use_poly_fusion:
                    f_cluster_irreps = self.cluster_pool(
                        f_node_irreps=f_node_irreps.reshape(-1, L, D),
                        f_node_pos=f_node_pos,                        # [B*N, 3]
                        f_cluster_pos=f_cluster_pos,                  # [B*max_clusters, 3]
                        flat_cluster_ids=flat_cluster_ids             # [B*N]
                    )                    
                else:
                    f_cluster_irreps = scatter_mean(f_node_irreps.reshape(-1, L*D), flat_cluster_ids, dim=0).reshape(-1, L, D)  # [B*max_clusters, D]       
                ...
                f_cluster_irreps = self.norm_ffn_cluster[i](f_cluster_irreps)
                if self.drop_path is not None:
                    f_node_irreps = self.drop_path(f_node_irreps,f_batch)
                f_node_irreps = f_node_irreps_res + f_node_irreps

        if self.pre_norm_concat:
            f_node_irreps_short_norm = self.norm_cluster_short(f_node_irreps_short)
            f_node_irreps_norm = self.norm_cluster_short(f_node_irreps)
            f_node_irreps_final = self.final_linear(torch.cat([f_node_irreps_short_norm,f_node_irreps_norm],dim=-1))
        else:
            # mean_short = f_node_irreps_short.mean()
            # std_short = f_node_irreps_short.std()
            # mean_long = f_node_irreps.mean()
            # std_long = f_node_irreps.std()
            # print("f_node_irreps_short", mean_short.item(), std_short.item())
            # print("f_node_irreps_long", mean_long.item(), std_long.item())
            # f_node_irreps_final = self.final_linear(
            #                     self.norm_cluster(torch.cat([f_node_irreps_short,f_node_irreps],dim = -1))
            #                     )
...
class E2formerClusterRdkit(torch.nn.Module):
    def __init__(
        self,
        irreps_node_embedding="128x0e+64x1e+32x2e",
        num_layers=6,
    ...
    def forward(
        ...
        **kwargs,
        ...
        ### bulid cluster embeddings
        B, N, L, D = node_irreps.shape 
        cluster_ids = batched_data["cluster_ids"]             # [B, N]
        cluster_pos = batched_data["cluster_centers"]         # [B, max_clusters, 3]
        max_clusters = cluster_pos.shape[1]

        masked_cluster_ids = cluster_ids.masked_fill(~node_mask, -1)
        num_clusters = (masked_cluster_ids.amax(dim=1).clamp(min=-1) + 1)
        cluster_range = torch.arange(max_clusters, device=cluster_ids.device).unsqueeze(0)
        cluster_mask = cluster_range < num_clusters.unsqueeze(1)
